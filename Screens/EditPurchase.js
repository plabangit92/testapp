...
const EditPurchase = ({ navigation,id }) => {
  ...
  const getPurchase = async () => {
    ...
    //   setEmployeeId(employee_id);
      const response = await axios.get(
        `https://computerworldgroup.com/track/api/purchase/editpurchase?id=${id}`,
      ...
      if (response.status === 200) {
        ...
        setitem_name(response.data.get_purchase_for_edit.item_name);
        setquantity(response.data.get_purchase_for_edit.quantity);
        setprice(response.data.get_purchase_for_edit.price);
