...
const NewPurchase = ({ navigation }) => {
  ...
  const onchangeSupplierName = (supplier_name) => {
    setSupplierName(supplier_name);
  };

  const fetchInitialPurchase = async () => {
    setIsLoading(true)
    try {
      ...
      const response = await axios.get(
        `https://computerworldgroup.com/track/api/purchase/initialpurchase?employee_id=${employee_id}`,
      ...
      if (response.status === 200) {
        ...
        setGetData(response.data.get_initialpurchase);
  ...
  const submitPurchase = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://computerworldgroup.com/track/api/purchase`,
        {
          employee_id,
          supplier_name,
          item_name,
      ...
      if (response.status === 200) {
        ...
        ToastAndroid.show("Item Purchased Successfully", ToastAndroid.SHORT);
  ...
  return (
    <ScrollView nestedScrollEnabled={true} style={{ width: "100%" }}>
      <View style={{ flex: 1 }}>
        ...
        <View>
          <TouchableOpacity
            onPress={() => navigation.navigate("AddPurchaseForm")}
          >
            <LinearGradient
              style={{
                height: 40,
        ...
        >
          ...
          <TextInput
            ...
            value={supplier_name}
            onChangeText={onchangeSupplierName}
            placeholder="Enter Supplier Name"
          />
        </View>

        <View>
          <TouchableOpacity onPress={submitPurchase}>
            <LinearGradient
              style={{
                height: 40,
                padding: 8,
